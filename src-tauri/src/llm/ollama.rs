...
pub async fn stream(
    ...
    let resp = client
        ...
        .json(&body)
        .send().await
        .map_err(|e| AtelierError::new(ErrorCode::ProviderUnavailable, format!("Cannot reach Ollama at {url}: {e}")))?;

    if !resp.status().is_success() {
        let status = resp.status().as_u16();
        let body = resp.text().await.unwrap_or_default();
        return Err(AtelierError::new(ErrorCode::ProviderError, format!("Ollama error {status}: {body}")));
    }
