...
pub async fn embed_texts(texts: &[&str], provider: &str, api_key: &str) -> Result<Vec<Vec<f32>>> {
    match provider {
        "openai" => embed_openai(texts, api_key).await,
        "ollama" => embed_ollama(texts, api_key).await,
        _ => Err(AtelierError::new(
...
async fn embed_openai(texts: &[&str], api_key: &str) -> Result<Vec<Vec<f32>>> {
    ...
    if !resp.status().is_success() {
        ...
        return Err(AtelierError::new(
            ...
            format!("OpenAI embed {status}: {body}"),
        ));
    }
...
async fn embed_ollama(texts: &[&str], base_url: &str) -> Result<Vec<Vec<f32>>> {
    ...
    for text in texts {
        ...
        let resp = client.post(&url).json(&body).send().await.map_err(|e| {
            AtelierError::new(ErrorCode::EmbeddingFailed, format!("Ollama embed: {e}"))
        })?;
        ...
        let vec: Vec<f32> = json["embeddings"][0]
            .as_array()
            .unwrap_or(&vec![])
            .iter()
            .map(|v| v.as_f64().unwrap_or(0.0) as f32)
