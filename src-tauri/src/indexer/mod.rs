...
static SKIP_EXTS: &[&str] = &[
    "exe", "dll", "so", "dylib", "bin", "obj", "o", "a", "lib", "png", "jpg", "jpeg", "gif", "svg",
    "ico", "webp", "bmp", "mp3", "mp4", "wav", "ogg", "avi", "mov", "mkv", "zip", "tar", "gz",
    "bz2", "xz", "7z", "rar", "woff", "woff2", "ttf", "eot", "db", "sqlite", "sqlite3",
];
...
pub async fn run_indexer(app: AppHandle, db: Db, workspace_id: i64, workspace_path: String) {
    ...
    if let Ok(db) = db.lock() {
        let _ = db.execute(
            "UPDATE workspaces SET index_status = 'complete' WHERE id = ?1",
            [workspace_id],
        );
    }
    let _ = app.emit(
...
async fn index_workspace(
    ...
    for (i, path) in files.iter().enumerate() {
        ...
        let text = match String::from_utf8(bytes) {
            Ok(t) => t,
            Err(_) => {
                upsert_file_skip(db, workspace_id, &rel_path, path, &meta, "binary").await;
                continue;
            }
        };
        ...
        {
            let Ok(db) = db.lock() else { continue };
            let _ = db.execute("DELETE FROM chunks WHERE file_id=?1", [file_id]);
        }
...

        {
            let Ok(db) = db.lock() else { continue };
            let _ = db.execute(
                "UPDATE files SET index_state='indexed',indexed_at=?1,content_hash=?2 WHERE id=?3",
        ...
        if let Some((ref provider, ref key)) = embed_creds {
            if pending_embed.len() >= EMBED_BATCH {
                flush_embeddings(db, std::mem::take(&mut pending_embed), provider, key).await;
            }
        }
    }
...
async fn flush_embeddings(db: &Db, batch: Vec<(i64, String)>, provider: &str, key: &str) {
    ...
    let Ok(embeddings) = embedder::embed_texts(&texts, provider, key).await else {
        return;
    };
