...
use crate::models::{FileNode, WorkspaceFile};
use std::fs;
...
fn build_tree(root: &Path, base: &Path) -> Vec<FileNode> {
    ...
    for entry in entries {
        ...
        let size_bytes = meta
            .as_ref()
            .and_then(|m| if !is_dir { Some(m.len() as i64) } else { None });
        let ext = if !is_dir {
            path.extension()
                .and_then(|e| e.to_str())
                .map(|s| s.to_string())
...
pub fn image_attachment_save(
    ...
    let bytes = base64::engine::general_purpose::STANDARD
        ...
        .map_err(|e| {
            AtelierError::new(ErrorCode::Unsupported, format!("Invalid image data: {e}"))
        })?;
    if bytes.len() > MAX_IMAGE_SIZE {
        return Err(AtelierError::new(
            ErrorCode::FileTooLarge,
            format!(
                "Image is {} MB, max is {} MB",
                bytes.len() / 1_000_000,
...
pub fn file_read_raw(workspace_id: i64, rel_path: String, db: State<Db>) -> Result<String> {
    ...
    if meta.len() > MAX_FILE_SIZE {
        return Err(AtelierError::new(
            crate::error::ErrorCode::FileTooLarge,
...
pub fn file_read_office_preview(
    ...
    let bytes =
        fs::read(&target).map_err(|e| AtelierError::io(format!("Cannot read {rel_path}: {e}")))?;
...
pub fn file_export_pdf(workspace_id: i64, rel_path: String, db: State<Db>) -> Result<String> {
    ...
    let pdf_rel_path = Path::new(&rel_path)
        .with_extension("pdf")
        .to_string_lossy()
        .to_string();
